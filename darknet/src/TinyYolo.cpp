...
TinyYolo::TinyYolo(char* cfg_f, char* wt_f) {
	read_config(cfg_f);
	config_init();
	print_network();
	allocate_tensors();
	configure_tensors();
	allocator_allocate();
	load_weights_biases_bnparams(wt_f, false);
}

float TinyYolo::linear_activate(float x) {
	return x;
}
float TinyYolo::logistic_activate(float x) {
	return 1. / (1. + exp(-x));
}

float TinyYolo::relu_activate(float x) {
	return x * (x > 0);
}
float TinyYolo::leaky_activate(float x) {
	return (x > 0) ? x : .1 * x;
}

float TinyYolo::activate(float x, ACTIVATION a) {
	switch (a) {
	case LINEAR:
...
void TinyYolo::activate_tensor(Tensor& src, ACTIVATION a) {
	Window input_window;
...
TinyYolo::ACTIVATION TinyYolo::string_to_act(string str) {
...
TinyYolo::layer_type TinyYolo::string_to_layer(string str) {
...
bool TinyYolo::isFloat(string myString) {
...
void TinyYolo::get_float_list(int l, string str) {
...
void TinyYolo::config_parser(string str) {
	switch (str[0]) {
	case '\n': {
		break;
	}
...
void TinyYolo::dump(Tensor& src, char* filename) {
	int p = 0;
	Window input_window;
	input_window.use_tensor_dimensions(src.info());
...
image TinyYolo::load(Tensor& src, char* filename) {
	char buff[256];
	char *input = buff;
...
void TinyYolo::dump_vector(vector<float>&vec, char* filename) {
	cout << filename << endl;
	FILE* fp = fopen(filename, "w");
...
void TinyYolo::dump_float(float* data, int size, char* filename) {
	cout << filename << endl;
	FILE* fp = fopen(filename, "w");
...
void TinyYolo::initialize_wb(Tensor& wb, FILE *fp) {
	Window input_window;
...
void TinyYolo::initialize_batchnorm_param(vector<float>& a, FILE* fp,
		int size) {
	float temp = 0;
	for (int i = 0; i < size; i++) {
		fread(&temp, sizeof(float), 1, fp);
...
void TinyYolo::vector2tensor(Tensor& b, vector<float> &bias) {
	Window input_window;
...
void TinyYolo::tensor2vector(Tensor& b, vector<float> &bias) {
	bias.clear();
	Window input_window;
	input_window.use_tensor_dimensions(b.info());
...
void TinyYolo::tensor2array(Tensor& b, float*bias) {
	Window input_window;
	input_window.use_tensor_dimensions(b.info());
	int c = 0;
	Iterator input_it(&b, input_window);
...
void TinyYolo::forward_batchnorm(Tensor& src, vector<float> biases,
...
int TinyYolo::entry_index(layer l, int batch, int location, int entry) {
...
void TinyYolo::activate_array(float* x, int n, ACTIVATION a) {
	int i;
	for (i = 0; i < n; ++i) {
		x[i] = activate(x[i], a);
	}
...
void TinyYolo::softmax(float *input, int n, float temp, int stride,
		float *output) {
	int i;
	float sum = 0;
...
void TinyYolo::softmax_cpu(float *input, int n, int batch, int batch_offset,
...
float* TinyYolo::forward_region_layer(layer l) {
...
box TinyYolo::get_region_box(float *x, float* biases, int n, int index, int i,
...
void TinyYolo::get_region_boxes(layer l, float* outputs, int w, int h,
		float thresh, float **probs, box *boxes, int only_objectness, int *map,
		float tree_thresh) {
	int i, j, n, z;
...
image TinyYolo::load_image(char* sf) {
	image im = load(TinyYolo::src, sf);
	return im;
}

void TinyYolo::load_weights_biases_bnparams(char* wf, bool dump) {
...
void TinyYolo::allocate_tensors() {
...
void TinyYolo::configure_tensors() {
	for (int l = 0; l < 6; ++l) {
		Tensor* temp;
		if (l == 0)
			temp = &src;
...
void TinyYolo::allocator_allocate() {
...
void TinyYolo::batch_norm_run(int i) {
...
void TinyYolo::activator_run(int i) {
	if (i != 8)
...
void TinyYolo::tiny_yolo_run(image im) {
	...
	char* names[] = { (char *) "aeroplane", (char *) "bicycle", (char *) "bird",
			(char *) "boat", (char *) "bottle", (char *) "bus", (char *) "car",
			(char *) "cat", (char *) "chair", (char *) "cow",
			(char *) "diningtable", (char *) "dog", (char *) "horse",
			(char *) "motorbike", (char *) "person", (char *) "pottedplant",
			(char *) "sheep", (char *) "sofa", (char *) "train",
			(char *) "tvmonitor" };
	...
	cout << "predictions saved at predictions.png" << endl;
	free(probs);
	free(boxes);
}

void TinyYolo::print_network() {
	for (unsigned int i = 0; i < layers.size(); ++i) {
...
void TinyYolo::read_config(char* cfg_f) {
	fstream file(cfg_f);
	int i = 0;
	string str;
