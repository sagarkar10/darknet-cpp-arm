...
void activate_tensor(Tensor& src, ACTIVATION a) {
	...
	//cout << input_window.x().end() << input_window.y().end()<< input_window.z().end();
...
layer_type string_to_layer(string str) {
	if (!str.compare("convolutional"))
		...
		return REGION;
	else if (!str.compare("softmax"))
		return SOFTMAX;
	else if (!str.compare("connected"))
		return CONNECTED;
	else {
		cout << "WRONG VALUE....:" << str << endl;
		exit(0);
...
void printlayer(layer l) {
	cout << "type:" << l.type << endl << "filters:" << l.filters << endl
			<< "in_c:" << l.in_c << endl << "out_c:" << l.out_c << endl
			<< "in_h:" << l.in_h << endl << "out_h:" << l.out_h << endl;
}
...
//			if (n_layers==layers.size()-2)
//				n_layers++;
			layers[n_layers].type = string_to_layer(section);
			layers[n_layers].layer_id = n_layers;
//			if(n_layers==0)	layers[n_layers].prev_layer = NULL;
//			else if(n_layers==1)	layers[n_layers].prev_layer = &(layers[0]);
//			else if (n_layers>1) {
//				cout<<"SETTING prev_layer for layer_id: "<<layers[n_layers].layer_id<<endl;
//				layers[n_layers].prev_layer = &(layers[n_layers-1]);
//				cout<<"TO layer_id: "<<(layers[n_layers].prev_layer)->layer_id<<endl;
//			}
		}
		break;
	}
...
//		cout<<"Setting something:"<<endl;
		if (layers.size() > 0 && layers[n_layers].type != REGION) {
			...
			if (!key.compare("anchors")){
				layers[n_layers].anchors = (float*) calloc(layers[n_layers].num*2,sizeof(float));
...
void config_init() {
	for (unsigned int i = 0; i < layers.size(); ++i) {
		...
		if (layers[i].type == CONVOLUTIONAL) {
			if (layers[i].pad==1) layers[i].padding=layers[i].size/2;
			else layers[i].padding=0;
			layers[i].out_c = layers[i].filters;
			layers[i].out_h = (layers[i].in_h + 2*layers[i].padding - layers[i].size)/layers[i].stride +1;
			layers[i].out_w = (layers[i].in_w + 2*layers[i].padding - layers[i].size)/layers[i].stride +1;
		} else if (layers[i].type == MAXPOOL) {
			if (layers[i].pad==1) layers[i].padding=1;
			else layers[i].padding=(layers[i].size-1)/2;
			layers[i].out_c = layers[i].in_c;
			layers[i].out_h = (layers[i].in_h + 2*layers[i].padding) / (layers[i].stride);
			layers[i].out_w = (layers[i].in_w + 2*layers[i].padding) / (layers[i].stride);
		} else if (layers[i].type == REGION) {

//			layers[i].in_h = net.src_height;
//			layers[i].in_w = net.src_width;
			layers[i].in_c = layers[i].num*(layers[i].classes + layers[i].coords + 1);
			layers[i].out_c = (layers[i].in_c);
...
FILE* gfile = fopen("GlobalDump.txt", "w");
...
void dump(Tensor& src, char* filename) {
	...
	{
		//cout<<*reinterpret_cast<float *>(input_it.ptr())<<endl;
		p++;
...
image load(Tensor& src, char* filename) {
	...
	save_image_png(sized, "loaded_image");
	return sized;
}
...
void forward_batchnorm(Tensor& src, vector<float> biases, vector<float> scales,
		vector<float> mean, vector<float> variance, int batch, int n,
		int h, int w, bool onlybias) {
	Window input_window;
	input_window.use_tensor_dimensions(src.info());
	Iterator input_it(&src, input_window);
	for(int z = 0; z < n; ++z)
	     {
	       for(int i = 0; i < h; ++i)
	       {
	         ...
	         {
	        	if(!onlybias){
				   *reinterpret_cast<float*>( src.buffer() + src.info()->offset_element_in_bytes(Coordinates(x,i,z))) = ( *reinterpret_cast<float*>( src.buffer() + src.info()->offset_element_in_bytes(Coordinates(x,i,z)))- mean[z])/(sqrt(variance[z]) + .000001f);
				   *reinterpret_cast<float*>( src.buffer() + src.info()->offset_element_in_bytes(Coordinates(x,i,z)))*=scales[z];
	        	}
	           *reinterpret_cast<float*>( src.buffer() + src.info()->offset_element_in_bytes(Coordinates(x,i,z)))+=biases[z];
	         }
	       }
	     }
}
...
int entry_index(layer l, int batch, int location, int entry) {
	...
	return batch * l.out_h * l.out_c * l.out_w + n * l.in_w * l.in_h * (l.coords + l.classes + 1) + entry * l.in_w * l.in_h + loc;
}
...
//	float* output = (float*)calloc(n_out,sizeof(float));
	...
	softmax_cpu(input + index, l.classes, l.batch*l.num, n_in/l.num, l.in_w*l.in_h, 1, l.in_w*l.in_h, 1, output+index);
...
void get_region_boxes(layer l, float* outputs, int w, int h, float thresh, float **probs, box *boxes, int only_objectness, int *map, float tree_thresh)
{
    int i,j,n,z;
    ...
    for (i = 0; i < l.in_w*l.in_h; ++i){
        ...
        for(n = 0; n < l.num; ++n){
            ...
            boxes[index] = get_region_box(predictions, l.anchors, n, box_index, col, row, l.in_w, l.in_h, l.in_w*l.in_h);
            if(1){
                int max = w > h ? w : h;
            ...
            int class_index = entry_index(l, 0, n*l.in_w*l.in_h + i, 5);
                for(j = 0; j < l.classes; ++j){
                    int class_index = entry_index(l, 0, n*l.in_w*l.in_h + i, 5 + j);
...
//	unsigned int kernel_conv[10];
	for (int con = 0; con < 6; ++con) {
		...
		const TensorShape 			weights_shape_conv(kernel_x_conv, kernel_y_conv,layers[2*con].in_c, ofm_conv);
		const TensorShape 			biases_shape_conv(weights_shape_conv[3]);
		const TensorShape 			out_shape_conv(layers[2*con].out_h, layers[2*con].out_w, layers[2*con].out_c);
		weights[con].allocator()	->init(TensorInfo(weights_shape_conv, 1, DataType::F32));
		biases[con].allocator()		->init(TensorInfo(biases_shape_conv, 1, DataType::F32));
		out_conv[con].allocator()	->init(TensorInfo(out_shape_conv, 1, DataType::F32));
		out_act[con].allocator()	->init(TensorInfo(out_shape_conv, 1, DataType::F32));
		TensorShape 				out_shape_pool(layers[2*con+1].out_h, layers[2*con+1].out_w, layers[2*con+1].out_c);
		out_pool[con].allocator()	->init(TensorInfo(out_shape_pool, 1, DataType::F32));
	}
	...
	const TensorShape out_shape_conv7(layers[12].out_h, layers[12].out_w, layers[12].out_c);
	...
	const TensorShape out_shape_conv8(layers[13].out_h, layers[13].out_w, layers[13].out_c);
	...
	const TensorShape out_shape_conv9(layers[14].out_h, layers[14].out_w, layers[14].out_c);
	...
	const TensorShape weights_shape_fc1(
			out_shape_conv9.x() * out_shape_conv9.y() * out_shape_conv9.z(),num_labels);
	const TensorShape biases_shape_fc1(num_labels);
	const TensorShape out_shape_fc1(num_labels);
	weights[9].allocator()->init(
			TensorInfo(weights_shape_fc1, 1, DataType::F32));
	biases[9].allocator()->init(TensorInfo(biases_shape_fc1, 1, DataType::F32));
	out_fc1.allocator()->init(TensorInfo(out_shape_fc1, 1, DataType::F32));
	out_act[9].allocator()->init(TensorInfo(out_shape_fc1, 1, DataType::F32));

	const TensorShape out_shape_softmax(out_shape_fc1.x());
	...
	for (int l = 0; l < 6; ++l) {
		Tensor * temp;
		if (l==0)		temp = &src;
		else			temp = &out_pool[l-1];

		conv[l].configure(temp, &weights[l], NULL, &out_conv[l],
				PadStrideInfo(layers[2*l].stride, layers[2*l].stride, layers[2*l].pad, layers[2*l].pad));
...
//			ActivationLayerInfo(ActivationLayerInfo::ActivationFunction::LEAKY_RELU));
		pool[l].configure(&out_conv[l], &out_pool[l],
				PoolingLayerInfo(PoolingType::MAX, layers[2*l+1].size,
						PadStrideInfo(layers[2*l+1].stride, layers[2*l+1].stride, layers[2*l+1].pad, layers[2*l+1].pad)));
	}
	conv[6].configure(&out_pool[5], &weights[6], NULL, &out_conv[6],
				PadStrideInfo(layers[12].stride, layers[12].stride, layers[12].pad, layers[12].pad));
//	act[6].configure(&out_conv[6], &out_act[6],
//				ActivationLayerInfo(ActivationLayerInfo::ActivationFunction::LEAKY_RELU));
	conv[7].configure(&out_conv[6], &weights[7], NULL, &out_conv[7],
			PadStrideInfo(layers[13].stride, layers[13].stride, layers[13].pad, layers[13].pad));
//	act[7].configure(&out_conv[7], &out_act[7],
//					ActivationLayerInfo(ActivationLayerInfo::ActivationFunction::LEAKY_RELU));
	conv[8].configure(&out_conv[7], &weights[8], NULL, &out_conv[8],
			PadStrideInfo(layers[14].stride, layers[14].stride, layers[14].pad, layers[14].pad));
	...
	fprintf(stderr, "Loading weights from : tiny-yolo-voc.weights\n");
	FILE *fp = fopen("tiny-yolo-voc.weights", "rb");
	if (!fp)
		exit(1);
	...
	image im = load(src, "dog.jpg");
	printf("Setting tensor Biases and Weights : ... \n");
	for (int i = 0; i < 9; ++i) {

		initialize_wb(biases[i], fp);
		if(layers[conv_layers[i]].type==CONVOLUTIONAL && layers[conv_layers[i]].size==1){
			cout<<"skip loading BN param"<<endl;
		}
		else{
...
//		dump_vector(rolling_variance[i], dump_name);
	...
	fclose(fp);
//	dump(src, "dump/src.txt");
	clock_t time;
	time = clock();
	for (int i = 0; i < 9; ++i) {
		conv[i].run();
//		char dump_name[100];
//		sprintf(dump_name, "dump/out_conv%d.txt", i);
//		dump(out_conv[i], dump_name);
		vector<float> biases_vector;
		tensor2vector(biases[i], biases_vector);
		if (layers[conv_layers[i]].batch_normalize == 1) {
			...
			forward_batchnorm(out_conv[i], biases_vector, scales[i],
					rolling_mean[i], rolling_variance[i], layers[conv_layers[i]].batch, layers[conv_layers[i]].filters ,
					layers[conv_layers[i]].out_h , layers[conv_layers[i]].out_w, false);
//			sprintf(dump_name, "dump/out_batchnorm%d.txt", i);
//			dump(out_conv[i], dump_name);

		} else if(layers[conv_layers[i]].type==CONVOLUTIONAL && layers[conv_layers[i]].size==1 && layers[conv_layers[i]].batch_normalize != 1) {
			forward_batchnorm(out_conv[i], biases_vector, scales[i],
								rolling_mean[i], rolling_variance[i], layers[conv_layers[i]].batch, layers[conv_layers[i]].filters ,
								layers[conv_layers[i]].out_h , layers[conv_layers[i]].out_w, true);
//			cout << "Only adding bias in batch norm\n";
//			sprintf(dump_name, "dump/out_batchnorm%d.txt", i);
//						dump(out_conv[i], dump_name);
		}
		if(i==8)
...
//		sprintf(dump_name, "dump/out_act%d.txt", i);
//		dump(out_conv[i], dump_name);
		if (i < 6) {
			pool[i].run();
//			sprintf(dump_name, "dump/out_pool%d.txt", i);
//			dump(out_pool[i], dump_name);
		}
	}
	...
	for(j = 0; j < l.in_w*l.in_h*l.num; ++j) probs[j] = (float*)calloc(l.classes + 1, sizeof(float));

	float thresh=.4;
	float nms=0.4;
	get_region_boxes(l,output, 1, 1, thresh, probs, boxes, 0, 0, thresh);
	if (nms) do_nms_sort(boxes, probs, l.in_w*l.in_h*l.num, l.classes, nms);

	image** alphabet = load_alphabet();

	char* names[] ={"aeroplane","bicycle","bird","boat","bottle","bus","car","cat","chair","cow",
			"diningtable",
			"dog",
			"horse",
			"motorbike",
			...
			"train",
			"tvmonitor"};
	draw_detections(im, l.in_w*l.in_h*l.num, thresh, boxes, probs, names, alphabet, l.classes);
